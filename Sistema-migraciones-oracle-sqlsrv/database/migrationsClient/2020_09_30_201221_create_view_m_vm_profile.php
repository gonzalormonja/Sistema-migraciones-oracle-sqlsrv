...
class CreateViewMVmProfile extends Migration
{
    ...
    public function up()
    {
        ...
        if($isSqlServer){
            ...
            $sql=
            '
            CREATE OR REPLACE FORCE EDITIONABLE VIEW M_VM_PROFILE ("VM_METER_ID", "VM_CHANNEL_ID", "METER_T0", "METER_TF", "LOCAL_T0", "LOCAL_TF", "QUALIFIER", "VAL", "VAL_DEMAND", "VAL_EDIT", "VAL_FACTOR", "REPORT", "REG_QUALITY") AS 
            select 
            b.vm_meter_id, b.vm_channel_id,a.meter_t0, a.meter_tf, a.local_t0, a.local_tf, 
            sum(a.qualifier) as qualifier, sum(a.val*b.multiplier) as val,sum(a.val_demand*b.multiplier) as val_demand, 
            sum(a.val_edit*b.multiplier) as val_edit, sum(a.val_factor*b.multiplier) as val_factor, a.report, 
            sum(cast(a.reg_quality as int)) as reg_quality 
            from 
            m_profile a join (select e.vm_meter_id,e.vm_channel_id,e.ph_meter_id,e.ph_channel_id, e.multiplier, c.enable from vm_channel_assign e join vm_meter_assign c on e.ph_meter_id=c.ph_meter_id and e.vm_meter_id=c.vm_meter_id where c.enable=1) b on a.meter_id=b.ph_meter_id and a.reg_descr_id=b.ph_channel_id
            group by b.vm_meter_id, b.vm_channel_id, a.meter_t0, a.meter_tf, a.local_t0, a.local_tf, a.report
            ';
            DB::unprepared($sql);
        }
    }
