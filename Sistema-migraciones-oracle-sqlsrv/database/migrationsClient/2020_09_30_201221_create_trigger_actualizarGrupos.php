...
class CreateTriggerActualizarGrupos extends Migration
{
    ...
    public function up()
    {
        ...
        if($isSqlServer){
            ...
            $sql=
            "
            CREATE OR REPLACE TRIGGER ActualizarGrupos AFTER INSERT ON m_app_pa_data
            BEGIN
                update m_location set group_id = (SELECT T1.group_id FROM m_cnc T1
                INNER JOIN m_app_pa_data T2 ON  T1.cnc_id = T2.cnc_id
                INNER JOIN m_location T3  ON T2.meter_id=T3.meter_id
                WHERE  T3.group_id <> T1.group_id);
            END;
            ";
            DB::unprepared($sql);
            $sql = "ALTER TRIGGER ActualizarGrupos ENABLE";
            DB::unprepared($sql);
        }
    }
