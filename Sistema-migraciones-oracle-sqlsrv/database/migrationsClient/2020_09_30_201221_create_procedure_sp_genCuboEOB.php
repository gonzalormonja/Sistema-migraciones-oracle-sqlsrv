...
class CreateProcedureSpGenCuboEOB extends Migration
{
    ...
    public function up()
    {
        ...
        if($isSqlServer){
            ...
            $sql =
            "
            CREATE OR REPLACE PROCEDURE sp_GenCubeEOB
            (
                ArgDesde VARCHAR2 := '',
                ArgHasta VARCHAR2 := '',
                ArgTop   NUMBER := 0,
            ...
            BEGIN
                ...
                SELECT COUNT(*) INTO vFlagTableExists FROM USER_TABLES WHERE TABLE_NAME = 'D_REGISTEREOB';

                -- Desde
                IF (ArgDesde IS NULL) THEN
                    SELECT fn_CubeGetFromGen('EOB') INTO vDesde FROM dual;
                ELSE
                    vDesde := ArgDesde;
                END IF;
                ...
                FOR record IN c1 LOOP
                    ...
                    vROWCOUNT := 0;
                    SELECT COUNT(*) INTO vROWCOUNT FROM USER_TAB_COLS WHERE TABLE_NAME = 'D_REGISTEREOB' AND COLUMN_NAME = vSQLSELFASDESCRIPTION;
                    IF vROWCOUNT = 0 THEN
                        vDoRebuild := 1;
                    END IF;
                END LOOP;
                ...
                DECLARE
                    q int;
                BEGIN
                    SELECT count (*) INTO q FROM user_tables WHERE table_name = upper('D_REGISTEREOB');
                    CASE WHEN (vDoRebuild = 1 AND q > 0) THEN
                            EXECUTE IMMEDIATE 'DROP TABLE D_REGISTEREOB';   
                        WHEN q = 0 THEN
                ...
                IF vDoRebuild = 1 THEN
                    vSQL := 'CREATE TABLE D_REGISTEREOB ( ' || chr(10);
                    ...
                    vSQL := vSQL || 'CONSTRAINT PK_d_registerEOB PRIMARY KEY (Id), ' || chr(10);
                    vSQL := vSQL || 'CONSTRAINT UK_m_registerEOB_id UNIQUE (m_register_id) ' || chr(10);
                    vSQL := vSQL || ')';
                    EXECUTE IMMEDIATE vSQL;
                    EXECUTE IMMEDIATE 'CREATE INDEX IX_deob_Fecha_Hora ON D_REGISTEREOB (Fecha_Hora)';
                    EXECUTE IMMEDIATE 'CREATE INDEX IX_deob_Año ON D_REGISTEREOB (Año)';
                    EXECUTE IMMEDIATE 'CREATE INDEX IX_deob_Mes ON D_REGISTEREOB (Mes)';
                    EXECUTE IMMEDIATE 'CREATE INDEX IX_deob_Dia ON D_REGISTEREOB (Dia)';
                    EXECUTE IMMEDIATE 'CREATE INDEX IX_deob_Cliente ON D_REGISTEREOB (Cliente)';
                    EXECUTE IMMEDIATE 'CREATE INDEX IX_deob_Punto_de_Medida ON D_REGISTEREOB (Punto_de_Medida)';
                    EXECUTE IMMEDIATE 'CREATE INDEX IX_deob_Medidor ON D_REGISTEREOB (Medidor)';
                ELSE
                    -- (4B) Si no se creó la tabla, borro los valores que serán reemplazados, si los hubiera
                    vSQL := 'DELETE D_REGISTEREOB where 1 = 1 ';
                    ...
                    IF vFlagDoDeletePorEjecucionManual = 1 THEN
                        EXECUTE IMMEDIATE vSQL;
                    ELSE
                        SELECT fn_CubeGetPreLastExecute('EOB') INTO NDesdePC FROM dual;
                        EXECUTE IMMEDIATE 'DELETE FROM D_REGISTEREOB WHERE D_REGISTEREOB.m_register_id IN (SELECT m_register_id FROM m_register WHERE m_register.datetime_pc >= TIMESTAMP ''' || TO_CHAR(NDesdePC, 'YYYY-MM-DD HH24:MI:SS') || ''')';
                    END IF;
                END IF;

                --INSERTO LOS NUEVOS VALORES
                vSQL := '';
                vSQL := 'INSERT INTO D_REGISTEREOB (';
                ...
                CASE
                    WHEN (ArgTop IS NULL AND vDoRebuild = 1) THEN
                        SELECT max_reg_gen into vTop FROM d_cube_config WHERE main_filter = 'EOB';
                    WHEN ArgTop > 0 THEN
                        vTop := ArgTop;
                    ELSE
                        SELECT max_reg_gen into vTop FROM d_cube_config WHERE main_filter = 'EOB';
                ...
                vSQL := vSQL || ' WHERE SUBSTR(d.reg_type, -1, 1) = ''1'' AND ROWNUM <= ' || vTop || ' ';
                ...
                -- VERIFICO SI SE SOBREPASO EL LIMITE ESPECIFICADO DE TAMAÑO MAXIMO PARA TABLA CONSOLIDADA.
                -- DEBO EJECUTAR LA CONSULTA DE MANERA DINAMICA, YA QUE SI LA TABLA D_REGISTEREOB NO EXISTE AL MOMENTO DE EJECUTARSE
                -- ESTE SP, POR MAS QUE LA MISMA SEA CREADA EN EL MISMO, ORACLE REPORTARÁ ERROR EN EL CODIGO FUENTE DEL SP
                SELECT max_saving_reg INTO vMaxReg FROM d_cube_config WHERE main_filter = 'EOB';
                EXECUTE IMMEDIATE 'DELETE FROM D_REGISTEREOB WHERE rowid IN (SELECT rowid AS rid FROM (SELECT * FROM D_REGISTEREOB ORDER BY id ASC) where ROWNUM <= ((SELECT count(id) FROM D_REGISTEREOB) - ' || vMaxReg || ') AND ' || vMaxReg  ||  ' < (SELECT count(id) FROM D_REGISTEREOB))';
            END;
            ";
            DB::unprepared($sql);
        }
